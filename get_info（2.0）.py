...
def write_excel(file_neme,form_info):
    ...
    for index,li in enumerate(form_info):
        ...
        print(form_info[index]['major'])
        print(form_info[index]['time'])
        print(form_info[index]['url'])
        worksheet.write(index, 0, form_info[index]['name'][0])  # 向第一列写入name
        worksheet.write(index, 1, form_info[index]['phone'][0])  # 向第2列写入phone
        worksheet.write(index, 2, form_info[index]['major'][0])  # 向第2列写入phone
        worksheet.write(index, 3, form_info[index]['time'][0])  # 向第3列写入time
        worksheet.write(index, 4, form_info[index]['url'][0])  # 向第4列写入url
    workbook.close()
